...
The backend SHALL provide comment, review, like, share, and share-card APIs for orders.
...
- **Given** a client calls `POST /api/orders/:id/like`, `DELETE /api/orders/:id/like`, or `POST /api/orders/:id/share`
- **When** the order belongs to the caller's family
- **Then** the backend returns JSON confirming the current liked/share state or created share record
...
- **Given** a client calls `GET /api/orders/:id/share-card`
- **When** the order belongs to the caller's family
- **Then** the backend returns an object containing top-level `order`, `items`, and `likeCount`
...
The backend SHALL validate request payloads before applying state changes.
...
- **Given** route handlers throw `ZodError` or SQLite unique-constraint errors not already handled locally
- **When** the app error boundary processes the error
- **Then** the backend returns `400` for validation failures, `409` for uniqueness conflicts, and `500` for unhandled server failures
